...
fullOpts = info (options <**> helper) (
  fullDesc <> progDesc "Run a wrapped gopls with notebook support"
  )
...
main = do
  ...
  wrappedLanguageServerPath <- (pure optWrappedLanguageServer <|> findExecutable "gopls") >>= \case
    Nothing -> throwIO $ userError [i|Couldn't find gopls binary.|]
    Just x -> return x
